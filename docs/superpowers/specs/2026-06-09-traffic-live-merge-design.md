...
`la source trafic`. Deux défauts dans l'implémentation actuelle l'empêchent :
...
- **Live, zéro persistence.** Chaque `GET /traffic` refetch les tuiles depuis la source trafic et renvoie
...
- **Politique d'échec : best-effort.** Si une tuile échoue (timeout, réponse vide), on renvoie le
  ...
  `/traffic` reste utilisable même si la source trafic a un hoquet. (Le futur cron de persistence utilisera
...
# Template tuile .pbf
scrape.tile-template=${SCRAPE_TILE_TEMPLATE:https://traffic.example.com/tile/v1/car/tile({x},{y},{zoom}).mvt}
```
